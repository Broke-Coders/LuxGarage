...
public class RentalController : ControllerBase
{
    ...
    [Authorize]
    public async Task<ActionResult<ApiResponse<RentalResponse>>> Cancel(int id)
    {
        try
        {
            var userIdString = User.FindFirstValue(ClaimTypes.NameIdentifier);
            ...
            var rental = await _rentalService.CancelRentalAsync(id, userId, role);
            return Ok(ApiResponse<RentalResponse>.Ok(rental, "Rental cancelled successfully."));
        }
